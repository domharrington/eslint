...
SourceCode.prototype = {
    ...
    /**
     * Registers a fix that insert text at the specified index in the source text.
    ...
    /**
     * Registers a fix that inserts text after the specified range in the source text.
     * The fix is not applied until applyFixes() is called.
    ...
    insertTextAfter: function(range, text) {
        this.insertText(range[1], text);
    },

    /**
     * Registers a fix that inserts text before the specified range in the source text.
     * The fix is not applied until applyFixes() is called.
     * @param {int[]} range The range to replace, first item is start of range, second
     *      is end of range.
     * @param {string} text The text to insert.
    ...
    /**
     * Registers a fix that replaces text at the specified range in the source text.
     * The fix is not applied until applyFixes() is called.
     * @param {int[]} range The range to replace, first item is start of range, second
     *      is end of range.
     * @param {string} text The text to insert.
    ...
    /**
     * Registers a fix that removes the specified range of text from the source.
     * The fix is not applied until applyFixes() is called.
    ...
    /**
     * Applies the cached fixes to the source code and returns the result. Does not
     * alter the text property.
     * @returns {string} The fixed text.
     */
    applyFixes: function() {

        // sort in reverse order of occurrence
        this._fixes.sort(function(a, b) {
        ...
        this._fixes.forEach(function(fix) {
            text.splice(fix.range[0], fix.range[1] - fix.range[0], fix.text);
        });
