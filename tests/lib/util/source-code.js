...
describe("SourceCode", function() {
    ...
    describe("applyFixes()", function() {
        ...
        beforeEach(function() {
            sourceCode = new SourceCode(TEST_CODE, espree.parse(TEST_CODE, DEFAULT_CONFIG));
        });

        describe("insertText()", function() {

            it("should insert text at the end of the code", function() {
                sourceCode.insertText(TEST_CODE.length, "// done");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE + "// done");
            });

            it("should insert text at the beginning of the code", function() {
                sourceCode.insertText(0, "// start\n");
                var text = sourceCode.applyFixes();
                assert.equal(text, "// start\n" + TEST_CODE);
            });

            it("should insert text in the middle of the code", function() {
                sourceCode.insertText(13, "5 * ");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("6 *", "5 * 6 *"));
            });
        ...
        describe("insertTextAfter()", function() {
            ...
            it("should insert text at the end of the code", function() {
                sourceCode.insertTextAfter([0, TEST_CODE.length], "// done");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE + "// done");
            });

            it("should insert text in the middle of the code", function() {
                sourceCode.insertTextAfter([0, 13], "5 * ");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("6 *", "5 * 6 *"));
            });
            ...
            it("should insert text at the beginning, middle, and end of the code", function() {
                ...
                sourceCode.insertTextAfter([0, 13], "5 * ");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("6 *", "5 * 6 *") + "// done");
            });
        ...
        describe("insertTextBefore()", function() {
            ...
            it("should insert text at the beginning of the code", function() {
                sourceCode.insertTextBefore([0, 1], "// start\n");
                var text = sourceCode.applyFixes();
                assert.equal(text, "// start\n" + TEST_CODE);
            });

            it("should insert text in the middle of the code", function() {
                sourceCode.insertTextBefore([13, 14], "5 * ");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("6 *", "5 * 6 *"));
            });
        ...
        describe("replaceText()", function() {
            ...
            it("should replace text at the end of the code", function() {
                sourceCode.replaceText([0, 3], "let");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("var", "let"));
            });

            it("should replace text at the beginning of the code", function() {
                sourceCode.replaceText([4, 10], "foo");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("answer", "foo"));
            });

            it("should replace text in the middle of the code", function() {
                sourceCode.replaceText([13, 14], "5");
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("6", "5"));
            });
        ...
        describe("removeText()", function() {
            ...
            it("should remove text at the end of the code", function() {
                sourceCode.removeText([0, 4]);
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("var ", ""));
            });

            it("should remove text at the beginning of the code", function() {
                sourceCode.removeText([5, 10]);
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace("answer", "a"));
            });

            it("should remove text in the middle of the code", function() {
                sourceCode.removeText([14, 18]);
                var text = sourceCode.applyFixes();
                assert.equal(text, TEST_CODE.replace(" * 7", ""));
            });
        ...
        describe("combination", function() {
            ...
            it("should replace text at the beginning, remove text in the middle, and insert text at the end", function() {
                ...
                sourceCode.replaceText([0, 3], "let");
                var text = sourceCode.applyFixes();
                assert.equal(text, "let answer = 6;// done");
            });
